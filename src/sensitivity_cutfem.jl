...
function cutfem(case::String, x0s::Vector;plot_flag=false, time_flag=false, vtk_flag=false,γg=0.1)
method = "cutfem"
...
for order in orders
  ...
  for x₀ in x0s
    ...
    @timeit to "domain $order, $nₓ" begin
      Ω⁻, Ω⁻act, Γ₁, nΓ₁, Γ₂, nΓ₂, E⁰, nE⁰ = CaseSetup.build_domain(method, cutgeo, cutgeo_facets)
    end
...
if plot_flag
  ...
  CaseSetup.plot_L2(x0s, orders, l2s; title=method*" "*case*" L2 norm error")
  display(plt)

  plt2 = plot(legend=:bottomleft)
  CaseSetup.plot_cond(x0s, orders, cns; title=method*" "*case*" condition number")
  display(plt2)
end # if
if time_flag
  show(to)
